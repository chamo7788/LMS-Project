...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    if (image) {
      import(`../../../${image.url}`).then(imageModule => {
        setSelectedImage({
          ...image,
  ...
  return (
    <nav className='navbar'>
      ...
      <ul className={`navbar__menu ${isMobileMenuOpen ? 'navbar__menu--active' : ''}`}>
        ...
        <li className='navbar__menu-item'>
          <NavLink to="/services" onClick={toggleMobileMenu} className='navbar__menu-link' activeClassName="active">Services</NavLink>
        </li>
        <li className='navbar__menu-item'>
          <NavLink to="/contact" onClick={toggleMobileMenu} className='navbar__menu-link' activeClassName="active">Contact</NavLink>
        </li>
      </ul>
